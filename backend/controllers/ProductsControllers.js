...
class ProductsControllers {
    ...
    async getProductByCategory(req, res) {
        try {
            ...
            var con = mysql2.createConnection(configDB);// Tạo connection
            var products = await new Promise((resolve, rejects) => {
                con.query("SELECT * FROM products WHERE category = ?", [encodedCategory], function (err, result) {
                    if (err) rejects(err);
                    resolve(result)
                })
            })
...
`, function (err, result) {
    ...
    async editById(req, res) {
        try {
            ...
            var con = mysql2.createConnection(configDB);// Tạo connection
            var products = await new Promise((resolve, rejects) => {
                con.query(`UPDATE products SET title = "${data.title}", price = "${data.price}",description="${data.description}",category="${data.category}",
                image="${data.image}" WHERE id = "${data.id}" `, function (err, result) {
                    if (err) rejects(err);
                    resolve(result);
                });
            })
